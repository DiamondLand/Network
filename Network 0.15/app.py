...
from static.modules.Chekers.themeChecker import get_stype
            ...
            if all((SQL_Injection_Checker(name), SQL_Injection_Checker(contact), SQL_Injection_Checker(password))):
                if pcheck(password):
                    try:
                        ...
                        return flask.render_template('registration.html', script="registerFine()")
...
def get_page():
    ...
    return flask.render_template('page.html', info=info, theme=get_stype(info[9]))
...
def dev_contact():
    info = Database(path=pathDB).get_user_info(current_user.username, close=True)
    return flask.render_template('dev_contact.html', info=info, theme=get_stype(info[9]))
...
def dashbord():
    info = Database(path=pathDB).get_user_info(current_user.username, close=True)
    return flask.render_template('dashbord.html', info=info, theme=get_stype(info[9]))
...
def redactor():
    ...
    if flask.request.method == 'POST':
        ...
        if 'file' in flask.request.files and file.filename and FileNameToSave:
            ...
            Database(path=pathDB).updateMainUserInfo(current_user.username, path=os.path.join(app.config['UPLOAD_FOLDER'], FileNameToSave), description=flask.request.form['description'], close=True)
        else:  Database(path=pathDB).updateMainUserInfo(current_user.username, description=flask.request.form['description'], close=True)
        return flask.render_template('redactor.html', info=info, theme=get_stype(info[9]))
    else:
        return flask.render_template('redactor.html', info=info, theme=get_stype(info[9]))
...
def test():
    info = Database(path=pathDB).get_user_info(current_user.username, close=True)
    return flask.render_template('test.html', info=info)

@app.route("/changeTheme")
@login_required
def changeTheme():
    Database(path=pathDB).changeTheme(current_user.username, close=True)
    return flask.redirect(flask.url_for('get_page'))
